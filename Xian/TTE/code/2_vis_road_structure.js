...
const imageDir = '../data/road_structure_images';
if (!fs.existsSync(imageDir)) {
    fs.mkdirSync(imageDir);
}

const htmlDir = '../data/road_structure_trajectory_htmls';
...
function preFilterNodes(trajectoryData, bounds) {
    ...
    // Distance threshold
    const distanceThreshold = 0.0005; // 0.001 degrees, approximately 111 meters
...
async function loadGeoJsonData() {
    ...
    try {
        nodesGeoJsonCache = JSON.parse(fs.readFileSync('../data/road_structure_trajectory_htmls/porto_nodes.geojson', 'utf8'));
        console.log(`Loaded node data: ${nodesGeoJsonCache.features.length} nodes`);
        edgesGeoJsonCache = JSON.parse(fs.readFileSync('../data/road_structure_trajectory_htmls/porto_edges.geojson', 'utf8'));
...
async function main() {
    ...
    const trajectoryDir = "../data/road_structure_trajectory_jsons";
    ...
    const MAX_CONCURRENT_TASKS = 4; // Set maximum concurrent tasks
    let currentIndex = 0;
