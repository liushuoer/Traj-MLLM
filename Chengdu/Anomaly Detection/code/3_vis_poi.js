...
const imageDir = '../data/poi_anomaly_images';
if (!fs.existsSync(imageDir)) {
    fs.mkdirSync(imageDir);
}
const htmlDir = '../data/poi_anomaly_htmls';
...
<body>
    ...
    <script>
        // Initialize map
        const map = L.map('map').setView([41.15, -8.61], 17); // Default center point for Xi'an
        
        // Add tile loading status tracking variables
        let tilesLoading = 0;
        let tilesLoaded = 0;
        let mapReady = false;
        ...
        tileLayer.on('tileload', function(e) {
            ...
            console.log('Tile loading progress:', tilesLoaded + '/' + tilesLoading);
        });
        ...
        // Create method to check map loading status
...
async function renderAndCaptureTrajectory(trajectoryFilePath) {
    ...
    try {
        ...
        await page.waitForFunction(() => {
            return window.mapFullyLoaded === true;
        }, { timeout: 30000 });
...
async function main() {
    ...
    const trajectoryDir = "../data/ano_trajectory_jsons_poi";
